...
export class CircleCI {
  ...
  static getCheckConnectionInstance(
    config: CircleCIConfig,
    logger: AirbyteLogger
  ): CircleCI {
    // Returns an instance to check Connection, where project names are not updated
    if (!config.token) {
    ...
    const axiosInstance: AxiosInstance = this.getAxiosInstance(config, logger);
    // Only in the case of checkingConnection
    const result = new CircleCI(
      logger,
      axiosInstance,
      cutoffDays,
      config.max_retries ?? DEFAULT_MAX_RETRIES
    );
    result.setProjectNames(Array.from(config.project_names));
    return result;
  }
  ...
  setProjectNames(filtered_project_names: string[]): void {
    this.filtered_project_names = filtered_project_names;
  }
  ...
  static async instance(
    ...
    // Returns an instance with updated project names
    const circleCI_instance = CircleCI.getCheckConnectionInstance(
      config,
      logger
    );
    logger.info('Will get filtered proj names');
    const filtered_project_names = await this.getFilteredProjectNames(
      config,
      logger,
      circleCI_instance
    );
    circleCI_instance.setProjectNames(filtered_project_names);
    logger.info(
      'Got these filtered project names: ' +
        JSON.stringify(filtered_project_names)
    );
    config.filtered_project_names = filtered_project_names;
    return circleCI_instance;
  }

  static async getFilteredProjectNames(
    config: CircleCIConfig,
    logger: AirbyteLogger,
    circleCI: CircleCI
  ): Promise<string[]> {
    // Ensuring project names are correct
    let filtered_project_names: string[] = Array.from(config.project_names);
    const org_slug = await this.getOrgSlug(circleCI.axios, logger);
    logger.info(`Got org_slug ${org_slug}`);
    if (config.project_block_list) {
      logger.info(`Found project block list`);
      ...
      if (config.slugs_as_repos) {
        filtered_project_names = config.project_names.map(
          (s) => `${org_slug}/${s}`
        );
      }
    }
    return filtered_project_names;
  }
