...
import {CommandData, StreamSlice} from '../models';
import {WindsurfClient} from '../client';

export class Commands extends AirbyteStreamBase {
  constructor(
  ...
  getJsonSchema(): Dictionary<any, string> {
    return require('../../resources/schemas/commands.json');
  }
  ...
  ): AsyncGenerator<CommandData> {
    const client = WindsurfClient.instance(this.config, this.logger);
    const lastCutoff = streamState?.timestamp;
    
    yield* client.getCommandData(lastCutoff);
  }
