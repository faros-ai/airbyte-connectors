...
import {PCWData, StreamSlice} from '../models';
import {WindsurfClient} from '../client';

export class PCW extends AirbyteStreamBase {
  constructor(
  ...
  getJsonSchema(): Dictionary<any, string> {
    return require('../../resources/schemas/pcw.json');
  }
  ...
  ): AsyncGenerator<PCWData> {
    const client = WindsurfClient.instance(this.config, this.logger);
    const lastCutoff = streamState?.timestamp;
    
    yield* client.getPCWData(lastCutoff);
  }
